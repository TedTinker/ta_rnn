...
def char_to_one_hot(char):
    ...
    one_hot = torch.from_numpy(one_hot).to(torch.int32)
    return one_hot

# Function to convert a one-hot vector back to a character
def one_hot_to_char(one_hot_vector):
    index = np.argmax(one_hot_vector.detach().numpy())
    return index_to_char[index]

# Convert the cleaned text to a list of one-hot vectors
one_hot_vectors = torch.stack([char_to_one_hot(char) for char in text])

X = one_hot_vectors[:-1].unsqueeze(-1).clone().detach().squeeze(-1)
Y = one_hot_vectors[1:].unsqueeze(-1).clone().detach().squeeze(-1)



# 4: Single hidden state for each sequence
hidden_state = torch.from_numpy(np.random.randn(hidden_dim,)).to(torch.int32) * 0.01
